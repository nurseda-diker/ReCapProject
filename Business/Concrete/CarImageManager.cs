...
namespace Business.Concrete
{
    public class CarImageManager : ICarImageService
    {
        ...
        public IDataResult<List<CarImage>> GetyByCarId(int id)
        {
            var result = BusinessRules.Run(CheckIfCarImage(id));
            if (result != null)
            {
        ...
        public IDataResult<CarImage> GetyById(int imageId)
        {
        ...
        private IResult CheckIfCarImage(int id)
        {
            var result = _carImageDal.GetAll(c => c.CarId == id).Count;
            if (result>0)
            {
